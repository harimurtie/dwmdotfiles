...
--local filemanager = wibox.widget.imagebox(icons.image.files)
local filemanager = create_button("", x.color12, x.color8.."C0",x.color8.."E0")
filemanager:buttons(gears.table.join(
    awful.button({ }, 1, function ()
          awful.spawn.with_shell("~/bin/rangers")
    end)
))


--local youtube = wibox.widget.imagebox(icons.image.youtube)
local youtube = create_button("", x.color5, x.color8.."90", x.color8.."B0")
youtube:buttons(gears.table.join(
    awful.button({ }, 1, function ()
          awful.spawn.with_shell("~/bin/youtubes")
    end)
))

local koneksi = create_button("", x.color11, x.color8.."60",x.color8.."80")
koneksi:buttons(gears.table.join(
    awful.button({ }, 1, function ()
          awful.spawn.with_shell("~/bin/nmtuis")
    end)
))


-- local exit = wibox.widget.imagebox(icons.image.poweroff)
...
--local music = wibox.widget.imagebox(icons.image.music)
...
local sandwich = wibox.widget.imagebox(icons.image.dell)
...
awful.screen.connect_for_each_screen(function(s)
    ...
    -- Create the wibox
    s.mywibox = awful.wibar({screen = s, visible = true, ontop = true, type = "dock", position = "bottom", height = dpi(32)})
    --s.mywibox.height = beautiful.wibar_height
    --s.mywibox.height = dpi (5)
    ...
    -- Create the top bar
    s.mytopwibox = awful.wibar({screen = s, visible = true, ontop = false, type = "dock", position = "top", height = dpi(10)})
    -- Bar placement
    ...
    -- For "awful.wibar"
    -- s.padding = { bottom = s.padding.bottom + beautiful.screen_margin * 4 }
    -- For "wibox"
    -- s.padding = { bottom = s.mywibox.height + beautiful.screen_margin * 4 }
...
function wibars_toggle()
    ...
    s.mytopwibox.visible = s.mytopwibox.visible
end
