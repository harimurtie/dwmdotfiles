...
if [[ "$1" = "-s" ]]; then
    ...
    # Full screenshot
    maim -u -m 3 $FILENAME && convert $FILENAME  \( +clone -background black -shadow 1x1+0+0 \) +swap -background none -layers merge +repage $FILENAME && convert $FILENAME -bordercolor white -border 10 $FILENAME
    notify-send "Screenshot" --urgency low -i $PHOTO_ICON_PATH
fi
